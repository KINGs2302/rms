...
<main className="flex-1 p-5 flex-wrap gap-5 items-start overflow-y-auto">
    ...
    {loading ? (
      <div className="flex flex-wrap gap-5 p-5 w-full">
        {[...Array(6)].map((_, i) => (
          <Skeleton key={i} className="w-full sm:w-52 h-40 rounded-lg" />
        ))}
      </div>
    ) : (
      <div className="flex flex-wrap gap-5 p-5">
        {filteredMenu.map((item) => (
          ...
          >
            ...
            <p className="text-gray-600 text-sm">{item.category.category}</p>
            <p className="text-gray-700 text-sm">Quantity: {item.quantity} {item.parameter}</p>
